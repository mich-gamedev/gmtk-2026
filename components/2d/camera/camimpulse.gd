...
func _init(intensity: float = 20., angle: float = NAN, attack_time_sec: float = 0.05, decay_time_sec: float = .15, attack_trans: Tween.TransitionType = Tween.TRANS_CUBIC, decay_trans: Tween.TransitionType = Tween.TRANS_CUBIC) -> void:
	if !is_finite(angle): angle = randf() * TAU
